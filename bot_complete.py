...
def calc_stoch(clist, kp=14, sm=3):
    n=len(clist); if n<6: return 50.0,50.0; kv=[]
    for i in range(min(kp,n-1),n): sl=clist[max(0,i-kp+1):i+1]; h=max(x["high"] for x in sl); lo=min(x["low"] for x in sl)
    kv.append(100*(clist[i]["close"]-lo)/(h-lo) if h!=lo else 50.0)
    if not kv: return 50.0,50.0; d=float(np.mean(kv[-sm:])) if len(kv)>=sm else float(np.mean(kv))
...
async def smart_entry(asset_id, direction, expiry_minutes):
    ...
    try:
        ...
        open_price = candles[-1]["open"]; current_price = candles[-1]["close"]
        ...
        if seconds_to_wait > 5: await asyncio.sleep(seconds_to_wait); return True, "enter_new_candle"
        else: return False, "candle_ending_skip"
    except: return False, "error"

# ═══ السيرفر المتقدم (Threading) ═══
class Handler(BaseHTTPRequestHandler):
    ...
    def cors(self):
        ...
        self.send_header("Connection", "close") # لمنع تجميد الاتصال
    ...
    def do_GET(self):
        ...
        if path == "/" or path == "/tokyo52.html":
            try:
                filepath = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tokyo52.html")
            ...
            except: self.reply({"error": "HTML file not found"}, 404)
            return
    ...
    def do_POST(self):
        ...
        if path == "/login":
            ...
            else: USERS_DB[email] = {"password": data.get("password"), "role": "user", "fname": data.get("fname", ""), "lname": data.get("lname", ""), "approved": False}; self.reply({"success": True, "message": "تم التسجيل، بانتظار موافقة المدير"})
...
async def bot_loop():
    ...
    try:
        ...
        else: bot_state["connected"] = False; print(f"❌ فشل الاتصال: {reason}", flush=True)
    except Exception as e: bot_state["connected"] = False; print(f"❌ خطأ في الاتصال: {e}", flush=True)
...
if __name__ == "__main__":
    print("=" * 58, flush=True); print("  ⚡ طوكيو 52 - المحرك الجبارة", flush=True); print("=" * 58, flush=True)
    threading.Thread(target=lambda: asyncio.run(bot_loop()), daemon=True).start()
    
    port_str = os.environ.get("PORT", "8080"); port = int(port_str) if port_str.isdigit() else 8080
    try:
        # استخدام ThreadingHTTPServer لعدم تجميد الموقع
        srv = ThreadingHTTPServer(("0.0.0.0", port), Handler)
        print(f"✅ سيرفر الموقع المتعدد يعمل على البورت {port}", flush=True)
        srv.serve_forever()
    except Exception as e: print(f"❌ فشل تشغيل السيرفر: {e}", flush=True)
